...
; --- LADESCREENS & MENÜS ---
global GTA_MAINMENU := "|<onlinemainmenublackwhite>000000-0.90$92."
global GTA_MAINMENU_ALT := "|<storyblackwhite>E1E1E1-0.90$81."
...
global SignalRGB_EXE := "SignalRgb.exe"
global SignalRGB_PATH := A_LocalAppData "\VortxEngine\SignalRgbLauncher.exe"
global FC_DIR := "C:\Program Files (x86)\FanControl"
global FC_EXE := FC_DIR "\FanControl.exe"

global FC_DEFAULT := FC_DIR "\Configurations\Default.json"
global FC_NIGHT   := FC_DIR "\Configurations\NACHTMODUS.json"
global FC_USER    := FC_DIR "\Configurations\userConfig.json"

;Fan Control always reloads values when starting. and it uses the last active profile to call these values. 
;so if u create and select a Default profile in FanControl you can just copy and paste some other profiles into default and restart fancontrol in order to change presets
;with some ini manupulation. 
...
Run, %SignalRGB_PATH%,, Min
SetFanControlProfile(FC_NIGHT)
ApplyRequiredGTASettings()
...
SetFanControlProfile(profilePath)
{
    global FC_EXE, FC_DIR, FC_DEFAULT

    if !FileExist(profilePath)
    {
        MsgBox, 16, Fehler, Profil nicht gefunden:`n%profilePath%
        return
    }

    if !FileExist(FC_DEFAULT)
    {
        MsgBox, 16, Fehler, Default nicht gefunden:`n%FC_DEFAULT%
        return
    }

    Process, Close, FanControl.exe
    WinWaitClose, ahk_exe FanControl.exe,, 5
    Sleep, 500

    FileCopy, %profilePath%, %FC_DEFAULT%, 1

    Run, %FC_EXE%, %FC_DIR%, Hide
    Sleep, 1500
}
...
$F2::
    ...
    Process, Close, RockstarErrorHandler.exe
    Process, Close, SocialClubHelper.exe
    Process, Close, %SignalRGB_EXE%
    RSleep(2000)
    RestoreOriginalGTASettings()
    SetFanControlProfile(FC_USER)
    ...
    ;RGB
    ;-------------------------------------------------
    ;Run, ms-settings:personalization-lighting
    ;Sleep, 20
